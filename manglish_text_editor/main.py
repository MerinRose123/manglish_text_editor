from transliterator import generate_word_list
from tkinter import *
...
class MyApp(Tk):
    ...
    def __init__(self):
        ...
        italic_btn.configure(command=self.change_italic)
    ...
    def translate(self, event):
        ...
        for element in word:
            if element.isalpha():
                ...
                result = self.translate_word(sub_string) if sub_string else ''
                final_result += result + element
                sub_string = ''
        if sub_string:
            final_result += self.translate_word(sub_string) if sub_string else ''
        self.text_editor.delete(1.0, tk.END)
